...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.99,
        help="Float number in [0, 1]: Quantile of the brightest pixels \
            in Hough transform to consider them lines.It determines threshold \
            to binarize Hough Transform. Default value is 0.99.",
    )
    parser.add_argument(
        "--min_cluster_size",
        "-mcs",
        ...
        help="If >= 1, it is min number of pixels in a cluster \
            ...
            If in [0, 1) it is considered as min percentage of pixels. \
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Max limit on resulted lines,so it shows <= 'limit' most obvious \
        lines. If None, unlimited.",
    )
    parser.add_argument(
        "--color",
        "-c",
        ...
        help="Name of color to draw lines. Default value is 'red'. \
            ...
            reference/ImageColor.html#color-names",
    )
    parser.add_argument(
        "--thickness",
        "-th",
        type=int,
        default=5,
        help="Thickness of lines in pixles. \
        Default value is 5.",
    )
    parser.add_argument(
        "--gradient",
        "-g",
        action="store_true",
        help="Flag to say that input image is already a \
        gradient map. If True it provides the image in Hough \
        Transform without preparations.",
    )
    parser.add_argument(
        ...
        default="laplacian",
        help="If gradient flag is False, it transforms image \
        to edge map before Hough Transform using this method.",
    )
    ...
    # Find lines
    ht = HoughTransform(
        args.image, compute_edges=None if args.gradient else args.edge_detection_method
    )
    ...
    print("Drawing lines...")
    output_img = ht.draw_lines(lines=lines, color=args.color, thickness=args.thickness)
