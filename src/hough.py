    ...
    def __init__(
        ...
        """
            ...
            1) image: 
                ...
                - or path to image in PIL accessible format (PNG, JPEG, TIFF, etc.).
            ...
            2) compute_edges:
                ...
                - None or False: considers that the 'image' is already an edge map. Does nothing.
        """
        if isinstance(image, np.ndarray):
            if len(image.shape) != 2:
                raise ValueError("If given image is numpy.ndarray it must be IN GRAYSCALE and have shape (h, w).")
            ...
            if isinstance(image, PIL_Image):
                ...
                raise ValueError(f"Unknown type of image: '{type(image)}'. Requires to be numpy.ndarray, PIL.Image or str")
...
########################
    ...
    def transform(self, image : np.ndarray = None):
        ...
        Parameters:
            ...
            By default (if None) it is gradient oughTransform.grad_ from initializing.
        Returns: 
    ...
    def find_lines(
        ...
        """
            ...
            Parameters:
                ...
                3) quantile: Quantile of the brightest pixels in hough transforms 
                to consider them lines.
                ...
                5) max_lines: limit on resulted lines, so it returns <= 'max_lines' most obvious lines.
                If None, unlimited.
        ...
        def lazy_lines(name, rotate_times):
            if name in self.lazy_transforms:
                ...
                # Normalize transform. It is important to prevent imbalance in scores 
            ...
            shape = (
                self.shape[rotate_times % 2], 
                self.shape[(rotate_times + 1) % 2]
            )
            hough = (hough - hough.min()) / (hough.max() - hough.min())
            ...
            lines = [self.rotate_line_90_(l, shape, rotate_times) for l in lines]
            return lines
        ...
        **find_lines_kwargs
        ...
        """
            ...
            Parameters:
                ...
                - numpy image in grayscale with shape=(h, w) 
                - or PIL.Image in any format
                By default (if None) it is oughTransform.image_ from initializing. 
                !! If not None, you must provide lines for this image because it was not initialized !!
                2) lines: list of tuples in format (x1, y1, x2, y2) containing two points of line.
                !! If image and lines are None it finds lines automatically using **find_lines_kwargs !!
        ...
        if image is None and lines is None:
            ...
            raise ValueError("You must initialize new HoughTransform to \
...
###########################
    ...
    def find_horizontal_down_lines_(self, hough, width, quantile, min_cluster_size, max_clusters=None):
        # Binarize transform
    ...
    def compute_edges_(self, method='sobel', sobel_size=5):
        ...
        elif method is None or method == False:
